...
from motor_driver_pkg.CANalyst2_canbus_handler import CanbusHandler 
from motor_driver_pkg.motor_controller import MotorController
# from utils.motor_controller import MotorController



def load_arm_config(file_name='configs/arm_bind.yaml'):
    ###TODO file_name直接是绝对路径###
    file_path =file_name
...
class ArmController:
    def __init__(self,side_names=["riaht_arm"],config_file='configs/arm_bind.yaml', can_channel=0, baud_rate=1000, auto_enable=True):
        self.ARM_CONFIG = load_arm_config(config_file)
        self.config ={}
        for side_name in side_names:
            self.config[side_name] = self.ARM_CONFIG[side_name]
            if not self.config[side_name]:
        ...
        for side_name in side_names:
            side = self.config[side_name]
            self.motor_id_list.extend(side['motor_ids'])
            self.joint_names_list.extend(side['joint_names'])
            if 'brake_motor_ids' in side:
                self.brake_motor_ids.update(side['brake_motor_ids'])
    ...
    def _initialize_can_and_motors(self):
        try:
            ...
            for side_name in self.config:
                matcher = self.config[side_name]['uuid_matcher']
                motor_ids = self.config[side_name]['motor_ids']
            ...
            motor_ids=self.ARM_CONFIG["legs"]['motor_ids']
            found_device = None
            if matcher in device_map:
                found_device = device_map[matcher]
            else:
            ...
            if dev_idx not in self.can_handlers:
                print(f"Initializing CAN device {dev_idx} (SN: {found_device['serial_number']}) for legs...")
                can = CanbusHandler(can_channel=self.can_channel, baud_rate=self.baud_rate, device_index=dev_idx)
                can.open()
                self.can_handlers[dev_idx] = can
                print(f"CAN device {dev_idx} opened on channel {self.can_channel} @ {self.baud_rate}kbps.")
    ...
    def _brkto_monitor_worker(self):
        ...
        When a motor reaches its target position and remains within range for 2 seconds, it stops the motor to engage the brake.
        该线程定期检查所有BRKTO任务的电机位置。
        1. 如果电机位置接近目标位置（误差小于500），开始计时。
        2. 如果电机在目标位置范围内持续2秒，发送停止命令以启用刹车。
        3. 使用线程锁确保对共享数据结构的安全访问。
        4. 当没有更多BRKTO任务时，线程自动退出。
        5. 提供异常处理以防止
        """
                ...
                with self.monitoring_lock:
                    for motor_id in list(self.brkto_tasks.keys()):
                        ...
                        if abs(current_pos - task['target']) < 500:
                            ...
                            elif time.time() - task['in_range_since'] >= 0.5:
                                print(
    ...
    def _send_motor_command(self, motor_id, target_pose):
        ...
        print(f"  Motor {motor_id} commanded to position {target_pose}.")

    # 辅助函数：按设备分组电机 dev_idx -> [motor_ids]
    def _group_motors_by_device(self, motor_ids_to_group):
        """
        Groups motor IDs by their associated CAN device index.
        Returns a dictionary mapping device indices to lists of motor IDs.
        """
